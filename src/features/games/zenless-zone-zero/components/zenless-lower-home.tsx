...
const fallbackUpdates = [
  { title: "绝区零分站首屏上线", desc: "New Eridu layout ready", time: "Just now", badge: "NEW" },
  { title: "代理人外观分类预热", desc: "星见雅、艾莲、妮可", time: "5h ago", badge: null },
  { title: "XXMI 指引入口更新", desc: "一键跳转安装教程", time: "1d ago", badge: null },
  { title: "直链下载体验优化", desc: "OSS 高速下载预留", time: "2d ago", badge: null },
] as const;
...
function ZenlessFeaturedCard({ game, index, mod }: { game: GameConfig; index: number; mod: SiteMod }) {
  const tag = ["HOT", "TRENDING", "NEW", "AGENT"][index % 4];
  const display = displayMods[index % displayMods.length];

  return (
    <Link href={`${game.nav.mods}/${mod.id}`} className="group block min-w-[190px] overflow-hidden border-4 border-black bg-white text-black shadow-[6px_6px_0px_0px_#000] transition duration-100 hover:-translate-y-1 hover:shadow-[9px_9px_0px_0px_#000]">
      <div className="relative h-[104px] overflow-hidden border-b-4 border-black bg-black">
        <Image src={mod.coverImage} alt={mod.title} fill sizes="210px" className="object-cover transition duration-500 group-hover:scale-105" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" />
        <span className={`absolute left-2 top-2 border-2 border-black px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.12em] text-black shadow-[3px_3px_0px_0px_#000] ${index % 3 === 0 ? "bg-[var(--neo-accent)]" : index % 3 === 1 ? "bg-[var(--neo-secondary)]" : "bg-[var(--neo-muted)]"}`}>{tag}</span>
      </div>
      <div className="space-y-1.5 p-2.5">
        <p className="text-[9px] font-black uppercase tracking-[0.16em] text-black/55">{display.character}</p>
        <h3 className="line-clamp-1 text-xs font-black uppercase leading-tight text-black">{display.title}</h3>
...
function ZenlessLatestUpdates({ game, mods }: { game: GameConfig; mods: SiteMod[] }) {
  ...
  return (
    <section className="border-4 border-black bg-[var(--neo-panel)] p-3 text-black shadow-[7px_7px_0px_0px_#000]">
      <div className="mb-2 flex items-center justify-between gap-2">
        <h3 className="text-xs font-black uppercase tracking-[0.16em]">Latest Updates</h3>
        <Link href={`${game.nav.mods}?sort=latest`} className="text-[9px] font-black uppercase tracking-[0.14em] underline decoration-2">View All</Link>
      </div>
      <div className="space-y-2">
        {updates.map((update, index) => (
          <div key={`${update.title}-${index}`} className="flex items-center gap-2 border-2 border-black bg-white/86 px-2 py-1.5 shadow-[3px_3px_0px_0px_#000]">
            <div className="flex size-7 shrink-0 items-center justify-center border-2 border-black bg-[var(--neo-secondary)] text-xs font-black">{index + 1}</div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-[11px] font-black leading-tight">{update.title}</p>
              <p className="truncate text-[9px] font-bold text-black/55">{update.desc}</p>
            </div>
            {update.badge ? <span className="border-2 border-black bg-[var(--neo-accent)] px-1.5 py-0.5 text-[8px] font-black shadow-[2px_2px_0px_0px_#000]">{update.badge}</span> : null}
            <span className="text-[9px] font-black text-black/48">{update.time}</span>
          </div>
        ))}
      </div>
    </section>
...
function ZenlessPopularCategories({ game }: { game: GameConfig }) {
  return (
    <section className="border-4 border-black bg-white p-3 text-black shadow-[7px_7px_0px_0px_#000]">
      <div className="mb-2 flex items-center justify-between gap-2">
        <h3 className="text-xs font-black uppercase tracking-[0.16em]">Popular Categories</h3>
        <Link href={game.nav.mods} className="text-[9px] font-black uppercase tracking-[0.14em] underline decoration-2">View All</Link>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {categories.map((category, index) => (
          <Link key={category.name} href={`${game.nav.mods}?query=${encodeURIComponent(category.query)}`} className={`flex items-center gap-2 border-2 border-black px-2 py-1.5 shadow-[3px_3px_0px_0px_#000] transition hover:-translate-y-0.5 ${index % 3 === 0 ? "bg-[var(--neo-accent)]" : index % 3 === 1 ? "bg-[var(--neo-secondary)]" : "bg-[var(--neo-muted)]"}`}>
            <span className="text-sm">{category.icon}</span>
            <span className="min-w-0">
              <span className="block truncate text-[10px] font-black leading-tight">{category.name}</span>
              <span className="block text-[8px] font-bold text-black/55">{category.count} mods</span>
            </span>
          </Link>
        ))}
      </div>
...
function ZenlessCreatorsBar() {
  return (
    <div className="flex items-center gap-3 overflow-hidden border-4 border-black bg-white px-3 py-2 text-black shadow-[7px_7px_0px_0px_#000]">
      <p className="shrink-0 text-[9px] font-black uppercase tracking-[0.2em] text-black/55">Trusted by Creators</p>
      <div className="flex min-w-0 flex-1 items-center gap-3 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {creators.map((creator, index) => (
          <div key={creator.name} className="flex shrink-0 items-center gap-2">
            <div className={`flex size-7 items-center justify-center rounded-full border-2 border-black text-[10px] font-black ${index % 3 === 0 ? "bg-[var(--neo-accent)]" : index % 3 === 1 ? "bg-[var(--neo-secondary)]" : "bg-[var(--neo-muted)]"}`}>{creator.name[0]}</div>
            <div>
              <p className="text-[10px] font-black leading-none">{creator.name}</p>
              <p className="mt-0.5 text-[8px] font-bold text-black/52">{creator.followers} Followers</p>
            </div>
          </div>
        ))}
      </div>
