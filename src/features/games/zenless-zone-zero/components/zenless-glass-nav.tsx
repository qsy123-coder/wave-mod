...
export function ZenlessGlassNav({ game, isLoggedIn }: ZenlessGlassNavProps) {
  ...
  const getSwitchHref = (gameItem: GameConfig) => gameItem.key === "wuthering-waves" ? "/" : gameItem.nav.home;

  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b-2 border-black/75 text-black shadow-[0_4px_0px_0px_rgba(0,0,0,0.7)]">
      <div className="absolute  inset-0 bg-[rgba(255,253,245,0.02)] backdrop-blur-sm" />
      <div className="relative mx-auto flex h-[58px] w-full max-w-[1680px] items-center gap-2 px-2 sm:px-3 lg:px-3 xl:px-4">
        <Link href={game.nav.home} className="group flex min-w-0 items-center gap-2">
          ...
          <div className="hidden leading-none sm:block">
            ...
            <p className="mt-0.5 text-[9px] font-black uppercase tracking-[0.2em] text-white/62">绝区零 MOD 分站</p>
          </div>
        </Link>
            ...
            <div className="absolute right-0 top-[calc(100%+10px)] grid w-56 gap-2 border-4 border-black bg-white p-3 text-black shadow-[8px_8px_0px_0px_#000]">
              {games.map((gameItem, index) => (
                ...
                >
                  ...
                  <span className={`text-[10px] tracking-[0.16em] ${gameItem.key === game.key ? "text-white/70" : "text-black/60"}`}>{gameItem.shortName}</span>
                </Link>
              ))}
            </div>
          </details>

          <Link href="/zenless-zone-zero/favorites" className="hidden h-9 items-center gap-1.5 border-2 border-black bg-white px-3 text-[11px] font-black uppercase tracking-[0.12em] text-black shadow-[3px_3px_0px_0px_#000] transition hover:-translate-y-0.5 hover:bg-white sm:inline-flex">
            <Heart className="size-3.5" />收藏
          </Link>

          {isLoggedIn ? (
            <form action={signOutAction}>
              <button type="submit" className="inline-flex h-9 items-center gap-1.5 border-2 border-black bg-white px-3 text-[11px] font-black uppercase tracking-[0.12em] text-black shadow-[3px_3px_0px_0px_#000] transition hover:-translate-y-0.5 hover:bg-white">
                <LogOut className="size-3.5" />退出
              </button>
            </form>
          ) : (
            <Link href={`/auth/login?mode=user&next=${encodeURIComponent(game.nav.home)}`} className="inline-flex h-9 items-center gap-1.5 border-2 border-black bg-white px-3 text-[11px] font-black uppercase tracking-[0.12em] text-black shadow-[3px_3px_0px_0px_#000] transition hover:-translate-y-0.5 hover:bg-white">
              <LogIn className="size-3.5" />登录
            </Link>
          )}

          <Link href={game.nav.mods} className="hidden h-9 items-center gap-1.5 border-2 border-black bg-[var(--neo-accent)] px-3 text-[11px] font-black uppercase tracking-[0.12em] text-black shadow-[3px_3px_0px_0px_#000] transition hover:-translate-y-0.5 hover:shadow-[5px_5px_0px_0px_#000] xl:inline-flex">
      ...
      <div className="relative mx-auto flex w-full max-w-[1680px] items-center gap-1.5 overflow-x-auto px-2 pb-2 sm:px-3 lg:hidden">
        {zenlessNavItems.map((item, index) => (
          <Link key={item.href} href={item.href} className={`shrink-0 border-2 border-black px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-black shadow-[3px_3px_0px_0px_#000] ${index % 2 === 0 ? "bg-[var(--neo-secondary)]" : "bg-white"}`}>
            {item.label}
          </Link>
        ))}
      </div>
