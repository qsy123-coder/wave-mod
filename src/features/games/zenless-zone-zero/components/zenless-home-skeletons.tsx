...
function ZenlessHeroBackgroundSkeleton() {
  return (
    <MotionReveal
      className="absolute inset-x-0 -bottom-20 -top-10 overflow-hidden bg-black"
      delay={0}
      y={0}
    >
        ...
        <div className="max-w-2xl pt-1">
          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            <MotionReveal delay={0.22} rotate={-1}>
              <SkeletonBar className="h-11 w-36 bg-[var(--neo-accent)]" />
            </MotionReveal>
            <MotionReveal delay={0.26} rotate={1}>
              <SkeletonBar className="h-11 w-32 bg-[var(--neo-secondary)]" />
            </MotionReveal>
          </div>
        </div>
        <MotionReveal
      ...
      <div className="mx-auto grid max-w-[1500px] gap-3 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-3">
          <MotionReveal delay={0.04} y={24} rotate={-1}>
            <div className="grid overflow-hidden border-2 border-black bg-white py-0 text-black shadow-[7px_7px_0px_0px_#000] sm:grid-cols-5">
              {Array.from({ length: 5 }).map((_, index) => (
                <SkeletonBar
                  key={index}
                  className="h-[58px] border-2 bg-white/90"
                />
              ))}
            </div>
          </MotionReveal>
          <section className="space-y-2">
            <div className="flex items-center gap-3">
              <MotionReveal delay={0.08} rotate={-1}>
                <SkeletonBar className="h-8 w-36 bg-[var(--neo-secondary)] shadow-[5px_5px_0px_0px_#000]" />
              </MotionReveal>
              <div className="h-px flex-1 bg-white/65 shadow-[0_1px_0px_#000]" />
              <MotionReveal delay={0.12} rotate={1}>
                <SkeletonBar className="h-5 w-16 bg-white/80" />
              </MotionReveal>
            </div>
                  ...
                  <article className="block min-w-[190px] overflow-hidden border-4 border-black bg-white text-black shadow-[6px_6px_0px_0px_#000]">
                    ...
                    <div className="space-y-1.5 p-2.5">
                      ...
                      <div className="flex items-center justify-between">
                        <SkeletonBar className="h-3 w-10 bg-[var(--neo-secondary)]" />
                        <SkeletonBar className="h-3 w-14 bg-white" />
                      </div>
                    </div>
                  </article>
                </MotionReveal>
          ...
          <MotionReveal delay={0.22} y={24} rotate={1}>
            <SkeletonBar className="h-12 w-full bg-[var(--neo-muted)] shadow-[7px_7px_0px_0px_#000]" />
          </MotionReveal>
          <MotionReveal delay={0.26} y={20} rotate={-1}>
            <SkeletonBar className="h-[52px] w-full bg-white shadow-[7px_7px_0px_0px_#000]" />
          </MotionReveal>
        </div>
            ...
            <div className="border-4 border-black bg-[var(--neo-panel)] p-3 text-black shadow-[7px_7px_0px_0px_#000]">
              ...
              <div className="space-y-2">
                {Array.from({ length: 4 }).map((_, index) => (
                  <SkeletonBar
                    key={index}
                    className="h-10 w-full bg-white/86 shadow-[3px_3px_0px_0px_#000]"
                  />
                ))}
              </div>
            </div>
            ...
            <div className="border-4 border-black bg-white p-3 text-black shadow-[7px_7px_0px_0px_#000]">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Array.from({ length: 6 }).map((_, index) => (
                  <SkeletonBar
                    key={index}
                    className="h-11 w-full bg-[var(--neo-secondary)] shadow-[3px_3px_0px_0px_#000]"
                  />
                ))}
              </div>
            </div>
