...
function RankBadge({ rank }: { rank: number }) {
  ...
  if (rank === 2) return <Crown className="size-4 text-slate-300" fill="#94A3B8" />;
  if (rank === 3) return <Crown className="size-4 text-amber-600" fill="#CD7F32" />;
  return (
    <span className="inline-flex size-5 items-center justify-center text-[10px] font-black text-slate-400 tabular-nums">
      {rank}
    </span>
  );
}
...
function TrendIndicator({ value, secondary }: { value: number; secondary: number }) {
  if (secondary === 0) return <span className="text-[10px] text-slate-600">—</span>;
  ...
  return <span className="inline-flex items-center gap-0.5 text-[10px] font-black text-slate-400"><TrendingUp className="size-3" />—</span>;
}
  ...
  return (
    <Link
      ...
      className={`grid grid-cols-[auto_auto_1fr_auto_auto] items-center gap-1.5 sm:gap-2 border-2 border-black px-1.5 sm:px-2 py-1.5 shadow-[1px_1px_0px_0px_#000] transition hover:-translate-y-0.5 ${
        hl ? "bg-[#07111f]/25" : "bg-[#050914]/25 hover:bg-[#0a1220]/40"
      }`}
    >
      <RankBadge rank={rank} />
      ...
      <div className="min-w-0">
        <p className="truncate text-[10px] sm:text-[11px] font-black text-white group-hover:text-[var(--neo-accent)] transition">
          {mod.title}
        </p>
        <p className="text-[8px] sm:text-[9px] font-bold text-slate-400">{mod.character}</p>
      </div>
      <div className="text-right">
        <p className="text-[10px] sm:text-[11px] font-black text-white tabular-nums">{compact(valueOf(mod))}</p>
        <p className="text-[8px] font-bold uppercase text-slate-400">{valueLabel}</p>
      </div>
...
function CreatorRow({
  ...
  return (
    ...
    >
      <RankBadge rank={rank} />
      <span className="relative size-7 sm:size-8 shrink-0 overflow-hidden rounded-full border-2 border-black bg-[#0a0f1a]">
        {creator.avatarUrl ? (
          <Image src={creator.avatarUrl} alt={creator.displayName} fill sizes="32px" className="object-cover" />
        ) : (
          <span className="flex size-full items-center justify-center text-xs font-black text-slate-600">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[8px] sm:text-[9px] font-bold text-slate-400">{creator.modCount} mods</p>
      </div>
      <div className="text-right">
        <p className="text-[10px] sm:text-[11px] font-black text-white tabular-nums">{compact(valueOf(creator))}</p>
        <p className="text-[8px] font-bold uppercase text-slate-400">{valueLabel}</p>
      </div>
    </Link>
  );
...
export function ZenlessRankingLeaderboard({
  ...
  return (
    <section className={`${panel} overflow-hidden backdrop-blur-[2px]`}>
      {/* Tabs */}
      ...
      {!isEmpty && (
        <div
          className={`hidden sm:grid items-center gap-2 border-b border-white/10 px-2.5 py-1.5 text-[8px] font-black uppercase tracking-[0.12em] text-slate-400 ${
      ...
      <div className="space-y-0.5 p-1">
        ...
        {activeTab === "creator-followers" &&
          creatorsByDownloads.slice(0, 20).map((c, i) => (
        ...
        {activeTab === "creator-reputation" &&
          creatorsByMods.slice(0, 20).map((c, i) => (
      ...
      {/* Footer */}
      <div className="flex items-center justify-between border-t-2 border-black px-2.5 py-1.5">
        <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.08em] text-slate-600">
