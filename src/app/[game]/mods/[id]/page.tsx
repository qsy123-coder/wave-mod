...
async function GameModDetailContent({ params }: PageProps) {
  ...
  const detailPath = `/${game.key}/mods/${mod.id}`;
    ...
    >
      ...
      <MotionReveal delay={0.02} rotate={-1}>
        <div className="inline-flex flex-wrap items-center gap-2 border-4 border-black bg-white px-4 py-2 text-sm font-black uppercase tracking-[0.14em] text-black shadow-[6px_6px_0px_0px_#000]">
          <Link href={game.nav.home}>{game.name} 首页</Link>
          <span>/</span>
          <Link href={game.nav.mods}>{game.name} MOD</Link>
          <span>/</span>
          <span>{mod.title}</span>
        </div>
      </MotionReveal>
      ...
      <div className="grid gap-4">
        <div className="flex flex-nowrap items-center justify-between gap-3 overflow-x-auto pb-1">
          ...
          <MotionReveal delay={0.08} y={18} rotate={1}>
            <div className="shrink-0"><HeroSectionNav compact /></div>
          </MotionReveal>
        </div>
            ...
            <MotionReveal delay={0.12} y={24} rotate={1}>
              <div className="neo-card-lg bg-[#fff8ef] p-4 text-black">
                <div className="flex h-full flex-col gap-4">
                  <div>
                    <div className="inline-flex items-center gap-2 border-4 border-black px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em] shadow-[4px_4px_0px_0px_#000]" style={{ background: game.theme.accent }}>
                      <Sparkles className="size-3.5" />{game.name} Summary
                    </div>
        ...
        <div className="min-w-0 space-y-8">
          <MotionReveal delay={0.16} y={24} rotate={-1}>
            <section className="neo-card-lg bg-[#fff8ef] p-6 text-black">
              <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="neo-label text-black/60">{game.name} MOD Overview</p><h2 className="mt-2 text-3xl font-black">详情说明</h2></div></div>
...
export default function GameModDetailPage({ params }: PageProps) {
  return <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><Suspense fallback={<ModDetailSkeleton />}><GameModDetailContent params={params} /></Suspense></div>;
}
