...
async function FavoritesContent() {
  await requireAuthUser("/favorites");
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Link href="/mods?sort=hot" className="neo-button-primary inline-flex items-center gap-2 px-5 py-3 text-sm font-black uppercase tracking-[0.14em]">
          继续挑选 MOD<ArrowRight className="size-4" />
        </Link>
      </div>
            ...
            <Card className="neo-card-lg p-6" style={{ background: "var(--neo-panel)" }}>
              <CardContent className="flex min-h-[320px] flex-col items-center justify-center gap-5 p-0 text-center text-black">
                ...
                <div className="space-y-2">
                  <p className="text-2xl font-black">你的收藏夹还是空的</p>
                  <p className="max-w-xl text-sm font-bold leading-7 text-black/75">去详情页点击“收藏 MOD”后，这里会自动同步你保存的内容。</p>
                </div>
                <Link href="/mods?sort=hot" className="neo-button-primary inline-flex items-center gap-2 px-5 py-3 text-sm font-black uppercase tracking-[0.14em]">先去挑 MOD<ArrowRight className="size-4" /></Link>
              </CardContent>
            </Card>
          </MotionReveal>
            ...
            <MotionReveal delay={0.14} y={24} rotate={1}>
              <Card className="neo-card-lg p-6" style={{ background: "var(--neo-accent)" }}>
                <CardContent className="space-y-3 p-0 text-black">
                  <p className="inline-flex items-center gap-2 text-sm font-black uppercase tracking-[0.14em]"><Sparkles className="size-4" />即将接入</p>
                  <ul className="space-y-3 text-sm font-bold leading-7 text-black/80"><li>• 收藏夹按角色聚合</li><li>• 最近浏览同步展示</li><li>• 已失效 MOD 提醒</li></ul>
                </CardContent>
              </Card>
            </MotionReveal>
          </div>
          ...
          {favorites.map((mod, index) => (
            <MotionReveal key={mod.id} delay={0.1 + index * 0.04} y={24} rotate={index % 2 === 0 ? -1 : 1}>
              <ModCard
                ...
                actions={(
                  <FavoriteButton id={mod.id} isFavorited isLoggedIn nextPath="/favorites" variant="destructive" favoriteLabel="收藏 MOD" unfavoriteLabel="移出收藏" pendingLabel="移除中" onSuccessMessage={{ favorite: "已加入收藏。", unfavorite: "已从收藏夹移除。" }} onSuccessDescription={{ favorite: "现在可以在收藏页快速找到它。", unfavorite: "这条 MOD 已从你的收藏页消失。" }} />
                )}
              />
            </MotionReveal>
          ))}
    ...
    <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      <MotionReveal delay={0.04} rotate={1}>
        <section className="inline-block border-4 border-black px-5 py-4 shadow-[8px_8px_0px_0px_#000]" style={{ background: "var(--neo-muted)" }}>
          ...
          <h1 className="mt-2 text-4xl font-black text-black">登录后查看你保存的 MOD</h1>
        </section>
      </MotionReveal>
