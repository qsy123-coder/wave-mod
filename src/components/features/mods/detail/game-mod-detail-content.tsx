...
type GameModDetailContentProps = {
  params: Promise<{ game: string; id: string }>;
  redirectDefaultGame?: boolean;
};

export async function GameModDetailContent({ params, redirectDefaultGame = true }: GameModDetailContentProps) {
  const { game: gameSlug, id } = await params;
  const game = getGameBySlug(gameSlug);

  if (redirectDefaultGame && gameSlug === defaultGameKey) {
    redirect(`/mods/${id}`);
  }
